...
- `mcp-server/` — 独立 MCP server，用于 Claude Code/Codex 操作 vault；优先通过插件 HTTP API 使用 Obsidian API，回退到文件系统；单一 knowledge folder 同时接受逻辑根相对路径与完整 vault 路径
...
- 完成任务后确保代码可运行、测试通过
- 计划文档等非代码变更也要推送到远端，用户通过 Obsidian vault 同步查看
