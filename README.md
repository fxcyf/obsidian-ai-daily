...
- **桌面输入体验** — 普通 `Enter` 换行，`Cmd+Enter` / `Ctrl+Enter` 发送；多行内容会自动增高输入框
...
- **知识整理 Agent** — 命令面板「整理知识库」，自动扫描 Raw/ 中未整理的笔记（每次最多 5 篇），提取核心观点并在 Wiki/ 中创建或更新对应条目，添加 tags、summary 和 wiki-link 关联
- **对话知识蒸馏** — 聊天侧边栏「蒸馏」按钮，从当前对话中提取事实性知识，自动匹配已有 Wiki 条目或创建新条目
...
- **Claude 指令连续性** — 本地与 Proxy Claude Code 在创建或恢复 transcript session 时都会重新注入 Vault、工具和 Harness system prompt，避免切换后端或重新启动 CLI 后只恢复聊天内容却丢失工作模式
...
- 命令面板搜索「整理知识库」，自动将 Raw/ 中的笔记整理��� Wiki/
- 聊天侧边栏点击「蒸馏」按钮，从对话中提取知识保存为 Wiki 条目
