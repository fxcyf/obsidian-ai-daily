...
## 2026-07-21 — 回复选区上下文菜单 (`a981ce6`)

- **问题**：选区保存只能通过回复右上角图钉触发，不符合桌面端右键和移动端长按后的操作习惯。
- **解决**：当选区完全位于同一条 AI 回复内时，接管该回复的上下文菜单，提供“保存选中内容到 Inbox”和“复制选中内容”；无有效选区时保留默认菜单，图钉继续作为移动端 WebView 不支持上下文菜单事件时的兼容入口。
- **验证**：选区边界相关测试 5/5 通过，生产构建成功；完整测试的 145 项中 144 项通过，唯一失败仍为本任务开始前已存在的 `ToolSearch` 策略子集断言。
- **教训**：扩展上下文菜单时只应在目标动作确实可用时阻止默认事件，并保留用户最常用的复制操作和平台兼容入口。
